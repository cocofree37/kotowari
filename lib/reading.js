...
export async function generateReading({ facts, concern, palmImage }) {
  ...
  if (!apiKey) return fallbackReading(facts, concern, palmImage);
...
// APIキー未設定時の簡易結果(動作確認用)
function fallbackReading(facts, concern, palmImage) {
  const theme = LIFE_PATH_TEXT[facts.lifePath];
  return {
    source: 'fallback',
    headline: `${facts.sunSign}の今日は${facts.today.moon.phase}`,
    overall: `ライフパスナンバー${facts.lifePath}の「${theme}」が持ち味です。今日は${facts.today.moon.phase}(月齢${facts.today.moon.age})。無理に動くより、自分の得意な形で一歩進めると流れに乗れます。(※ANTHROPIC_API_KEY未設定のため簡易表示です)`,
    concern: `${concern}については、${theme}を意識して、小さな行動から始めてみましょう。`,
    palm: palmImage
      ? { detected: false, life: 'APIキー設定後に解析されます', heart: '', head: '', fate: '' }
      : { detected: false, life: '', heart: '', head: '', fate: '' },
